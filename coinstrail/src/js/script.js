...
function closeModal() {
	$('body').removeClass('modal-open overlay-open');
...
$(document).ready(function () {
	...
	$(document).mousedown(function (e){
		if (!$('.js-dropdown-wrap').is(e.target) && $('.js-dropdown-wrap').has(e.target).length === 0) {
			close_dropdown();
		}
		if (!$('.js-stamping-wrap').is(e.target) && $('.js-stamping-wrap').has(e.target).length === 0) {
			$('.js-stamping-btn').removeClass('open');
	...
	$('.js-dropdown-btn').on('click', function () {
		if($(this).hasClass('open')) {
			...
			close_dropdown();
			$(this).addClass('open');
			$(this).closest().addClass('open');
			$(this).next('.js-dropdown-body').slideDown(200, 'linear');
		}
	});
	...
	var iOS = (/iPad|iPhone|iPod/.test(navigator.platform) ||
		...
		!window.MSStream
	if(iOS == true) {
		$('body').addClass('touch ios ios-' + iOS);
	...
	/*-----Stamping----*/
	$('.js-stamping-btn').on('click', function () {
		if($(window).width() >= 960) {
			if ($(this).hasClass('open')) {
				$(this).removeClass('open');
				$(this).closest('.stamping__wrap').next('.js-stamping-list').slideUp(200, 'linear');
			} else {
				close_all()
				$(this).addClass('open');
				$(this).closest('.stamping__wrap').next('.js-stamping-list').slideDown(200, 'linear');
			}
		}
	});
	/*---Stamping-END--*/
	...
	$('.js-sidebar-btn').on('click', function () {
		let data = $(this).attr('data-button');
		if($(this).hasClass('open')) {

			switch (true) {
				case data === 'main-btn':
					$('.js-sidebar-btn').removeClass('open open-second');
					$('.js-sidebar-btn').next().slideUp(200, 'linear');
					break;
				case data === 'main-sub-btn':
					$(this).removeClass('open');
					$(this).closest('.js-sidebar-list').prev('.js-sidebar-btn').removeClass('open-second');
			...
			switch (true) {
				case data === 'main-btn':
					...
					break;
				case data === 'main-sub-btn':
					$("[data-button='" + data + "']").removeClass('open');
					$("[data-button='" + data + "']").closest('.js-sidebar-list').removeClass('open-second');
					$("[data-button='" + data + "']").next().slideUp(200, 'linear');
					break;
			}
			$(this).closest('.js-sidebar-list').prev('.js-sidebar-btn').addClass('open-second');
